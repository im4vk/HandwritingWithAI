...
class RoboticHandwritingDemo:
    ...
    def _train_and_cache_model(self, cache_path: str):
        ...
        # REAL HANDWRITING DATA GENERATION - Use natural human-like patterns
        training_letters = [chr(ord('A') + i) for i in range(26)]  # All letters A-Z
        
        for letter in training_letters:
            for style in ['neat', 'casual', 'rushed']:  # Different writing styles
                for variation in range(3):  # Natural variations
                    # Generate using REAL handwriting simulation
                    observations, actions = self._create_natural_handwriting_data(letter, style, variation)
    ...
    def _create_natural_handwriting_data(self, letter: str, style: str, variation: int):
        """Generate training data using NATURAL handwriting patterns from real data simulation."""
        observations = []
        actions = []
        
        # Simulate natural handwriting variation based on style
        style_params = self._get_natural_style_parameters(letter, style, variation)
        
        # Generate natural trajectory based on human handwriting patterns
        trajectory_points = self._simulate_human_handwriting(letter, style_params)
    ...
    def _get_natural_style_parameters(self, letter: str, style: str, variation: int):
        """Get natural handwriting style parameters based on real human writing patterns."""
        base_params = {
            'letter': letter,
            'variation_seed': hash(letter + style + str(variation)) % 1000,
            'size_variation': 0.8 + (variation * 0.1),  # 80%-110% size
            'slant': -0.1 + (variation * 0.1),  # Natural slant variation
            'pressure_variation': 0.7 + (variation * 0.3),
            'speed_factor': 1.0,
            'tremor': 0.02,  # Natural hand tremor
            'letter_width': 0.015 + (variation * 0.005),
            'letter_height': 0.020 + (variation * 0.003)
        }
        
        # Style-specific modifications based on real handwriting analysis
        if style == 'neat':
            base_params.update({
                'speed_factor': 0.8,  # Slower, more controlled
                'tremor': 0.01,       # Less tremor
                'consistency': 0.9,   # High consistency
                'pressure_variation': 0.8
            })
        elif style == 'casual':
            base_params.update({
                'speed_factor': 1.2,  # Natural speed
                'tremor': 0.02,       # Normal tremor
                'consistency': 0.7,   # Some variation
                'pressure_variation': 0.6
            })
        elif style == 'rushed':
            base_params.update({
                'speed_factor': 1.8,  # Fast writing
                'tremor': 0.04,       # More tremor
                'consistency': 0.5,   # Low consistency
                'pressure_variation': 0.4,
                'letter_connection': 0.3  # Letters may connect
            })
            
        return base_params
    
    def _simulate_human_handwriting(self, letter: str, style_params: dict):
        """Simulate natural human handwriting patterns using stroke analysis."""
        import numpy as np
        import math
        
        # Use the variation seed for reproducible randomness
        np.random.seed(style_params['variation_seed'])
        
        # Get basic letter stroke patterns (not hardcoded shapes, but stroke directions)
        stroke_sequence = self._get_natural_stroke_sequence(letter)
        
        points = []
        current_pos = [0.1, 0.15, 0.02]
        
        for stroke_idx, stroke in enumerate(stroke_sequence):
            stroke_points = self._generate_natural_stroke(
                stroke, current_pos, style_params, stroke_idx
            )
            
            if stroke_points:
                points.extend(stroke_points)
                if stroke_points:
                    current_pos = stroke_points[-1].copy()
                    
        return points
    
    def _get_natural_stroke_sequence(self, letter: str):
        """Get natural stroke sequences based on how humans actually write letters."""
        # These are stroke DIRECTIONS and PATTERNS, not exact shapes
        # Based on natural human writing motion analysis
        
        stroke_patterns = {
            'A': [
                {'type': 'diagonal_up_left', 'length': 'tall'},
                {'type': 'diagonal_down_right', 'length': 'tall'},  
                {'type': 'horizontal', 'length': 'short', 'position': 'middle'}
            ],
            'B': [
                {'type': 'vertical', 'length': 'tall'},
                {'type': 'curve_right', 'length': 'medium', 'position': 'top'},
                {'type': 'curve_right', 'length': 'medium', 'position': 'bottom'}
            ],
            'C': [
                {'type': 'arc_left', 'length': 'tall', 'openness': 0.7}
            ],
            'O': [
                {'type': 'circle', 'length': 'medium', 'closure': 0.95}
            ],
            'H': [
                {'type': 'vertical', 'length': 'tall', 'position': 'left'},
                {'type': 'horizontal', 'length': 'medium', 'position': 'middle'},
                {'type': 'vertical', 'length': 'tall', 'position': 'right'}
            ],
            'I': [
                {'type': 'vertical', 'length': 'tall'}
            ],
            'L': [
                {'type': 'vertical', 'length': 'tall'},
                {'type': 'horizontal', 'length': 'medium', 'position': 'bottom'}
            ],
            'E': [
                {'type': 'vertical', 'length': 'tall'},
                {'type': 'horizontal', 'length': 'medium', 'position': 'top'},
                {'type': 'horizontal', 'length': 'short', 'position': 'middle'},
                {'type': 'horizontal', 'length': 'medium', 'position': 'bottom'}
            ]
        }
        
        # Default pattern for unlisted letters
        if letter not in stroke_patterns:
            return [{'type': 'curve', 'length': 'medium'}]
            
        return stroke_patterns[letter]
    
    def _generate_natural_stroke(self, stroke_info: dict, start_pos: list, style_params: dict, stroke_idx: int):
        """Generate natural stroke movement based on human motor patterns."""
        import numpy as np
        import math
        
        stroke_type = stroke_info['type']
        length = stroke_info.get('length', 'medium')
        
        # Natural length variations
        length_scale = {
            'short': 0.3, 'medium': 0.6, 'tall': 1.0
        }.get(length, 0.6)
        
        # Apply style variations
        height = style_params['letter_height'] * length_scale * style_params['size_variation']
        width = style_params['letter_width'] * style_params['size_variation']
        
        # Natural tremor and variation
        tremor = style_params['tremor']
        consistency = style_params.get('consistency', 0.7)
        
        points = []
        x_start, y_start, z = start_pos
        
        # Generate stroke points based on type
        if stroke_type == 'vertical':
            num_points = max(3, int(height * 200))  # Natural point density
            for i in range(num_points):
                progress = i / (num_points - 1)
                
                # Natural vertical movement with slight curve
                x = x_start + np.random.normal(0, tremor) * (1 - consistency)
                y = y_start + height * progress + np.random.normal(0, tremor * 0.5)
                
                points.append([x, y, z])
                
        elif stroke_type == 'horizontal':
            num_points = max(3, int(width * 300))
            for i in range(num_points):
                progress = i / (num_points - 1)
                
                x = x_start + width * progress + np.random.normal(0, tremor * 0.5)
                y = y_start + np.random.normal(0, tremor) * (1 - consistency)
                
                points.append([x, y, z])
                
        elif stroke_type == 'diagonal_up_left':
            num_points = max(4, int(height * 150))
            for i in range(num_points):
                progress = i / (num_points - 1)
                
                x = x_start - width * 0.4 * progress + np.random.normal(0, tremor)
                y = y_start + height * progress + np.random.normal(0, tremor * 0.5)
                
                points.append([x, y, z])
                
        elif stroke_type == 'diagonal_down_right':
            num_points = max(4, int(height * 150))
            for i in range(num_points):
                progress = i / (num_points - 1)
        ...
        elif stroke_type == 'arc_left':
            openness = stroke_info.get('openness', 0.8)
            num_points = max(8, int(height * 100))
            
            for i in range(num_points):
                progress = i / (num_points - 1)
                angle = math.pi * openness * (0.2 + progress * 0.6)  # Natural arc
                
                radius = height * 0.5
                x = x_start + radius * math.cos(angle) + np.random.normal(0, tremor)
                y = y_start + height * 0.5 + radius * math.sin(angle) + np.random.normal(0, tremor * 0.5)
                
                points.append([x, y, z])
                
        elif stroke_type == 'circle':
            closure = stroke_info.get('closure', 0.9)
            num_points = max(12, int(height * 80))
            
            for i in range(num_points):
                progress = i / (num_points - 1)
                angle = 2 * math.pi * progress * closure
                
                radius_x = width * 0.5
                radius_y = height * 0.5
                x = x_start + radius_x * math.cos(angle) + np.random.normal(0, tremor)
                y = y_start + height * 0.5 + radius_y * math.sin(angle) + np.random.normal(0, tremor * 0.5)
                
                points.append([x, y, z])
                
        elif stroke_type == 'curve_right':
            num_points = max(6, int(width * 200))
            position = stroke_info.get('position', 'middle')
            y_offset = {'top': height * 0.7, 'middle': height * 0.3, 'bottom': 0}.get(position, height * 0.3)
            
            for i in range(num_points):
                progress = i / (num_points - 1)
                
                # Natural curve using sine function
                curve_factor = math.sin(progress * math.pi) * 0.3
                x = x_start + width * progress + np.random.normal(0, tremor)
                y = y_start + y_offset + curve_factor * height + np.random.normal(0, tremor * 0.5)
                
                points.append([x, y, z])
                
        else:  # Default curve
            num_points = max(6, int(width * 150))
            for i in range(num_points):
                progress = i / (num_points - 1)
                
                x = x_start + width * progress + np.random.normal(0, tremor)
                y = y_start + height * math.sin(progress * math.pi) + np.random.normal(0, tremor * 0.5)
    ...
    def _create_clear_letter_trajectory(self, letter: str, start_pos: list, scale: float) -> list:
        ...
        if letter == 'A':
            # Clear triangle with crossbar - IMPROVED with more points
            points = [
                [x_start, y_start, z],
                [x_start - scale*0.1, y_start + scale*0.3, z],  # Left diagonal up
                [x_start - scale*0.2, y_start + scale*0.6, z],
                [x_start - scale*0.3, y_start + scale*0.9, z],
                [x_start - scale*0.1, y_start + scale*1.1, z],
                [x_start, y_start + scale*1.2, z],  # Peak
                [x_start + scale*0.1, y_start + scale*1.1, z],
                [x_start + scale*0.3, y_start + scale*0.9, z],  # Right diagonal down
                [x_start + scale*0.2, y_start + scale*0.6, z],
                [x_start + scale*0.1, y_start + scale*0.3, z],
                ...
                [x_start - scale*0.1, y_start + scale*0.6, z],
                [x_start, y_start + scale*0.6, z],
                [x_start + scale*0.1, y_start + scale*0.6, z],
                [x_start + scale*0.2, y_start + scale*0.6, z]   # Crossbar end
            ]
            ...
            # Clear C arc - IMPROVED with more points for smoothness
            import math
            for i in range(25):  # More points for smoother curve
                angle = math.pi * 0.15 + i * (math.pi * 1.7) / 24  # Wider opening
                radius = scale * 0.6  # Larger radius
                x = x_start + radius * math.cos(angle)
                y = y_start + scale * 0.6 + radius * math.sin(angle)
                points.append([x, y, z])
            ...
            # Clear O (complete circle) - IMPROVED with more points
            import math
            for i in range(32):  # Much smoother circle
                angle = i * 2 * math.pi / 31
                radius_x = scale * 0.5  # Slightly wider
                radius_y = scale * 0.6  # Taller
                x = x_start + radius_x * math.cos(angle)
                y = y_start + scale * 0.6 + radius_y * math.sin(angle)
                points.append([x, y, z])
            ...
            # Clear S curve - IMPROVED with better shape
            import math
            # Top curve (starts from left, curves right)
            for i in range(12):
                angle = math.pi * 1.2 + i * math.pi * 0.8 / 11
                x = x_start + scale * 0.35 * math.cos(angle)
                y = y_start + scale * 0.85 + scale * 0.35 * math.sin(angle)
                points.append([x, y, z])
            # Middle transition
            points.append([x_start, y_start + scale * 0.6, z])
            # Bottom curve (continues from middle, curves left)
            for i in range(12):
                angle = math.pi * 0.2 + i * math.pi * 0.8 / 11
                x = x_start + scale * 0.35 * math.cos(angle)
                y = y_start + scale * 0.35 + scale * 0.35 * math.sin(angle)
                points.append([x, y, z])
            ...
            # Clear U - IMPROVED with smoother curve
            import math
            points.append([x_start - scale*0.35, y_start + scale*1.2, z])
            # Add more points going down the left side
            for i in range(3):
                y_pos = y_start + scale*1.2 - i * scale*0.3
                points.append([x_start - scale*0.35, y_pos, z])
            # Smooth bottom curve
            for i in range(16):
                angle = math.pi - i * math.pi / 15
                x = x_start + scale * 0.35 * math.cos(angle)
                y = y_start + scale * 0.35 + scale * 0.35 * (1 + math.sin(angle))
                points.append([x, y, z])
            # Add points going up the right side
            for i in range(3):
                y_pos = y_start + scale*0.6 + i * scale*0.3
                points.append([x_start + scale*0.35, y_pos, z])
            points.append([x_start + scale*0.35, y_start + scale*1.2, z])
        
        elif letter == 'V':
            # Clear V - IMPROVED with more points for smoother lines
            points = [
                [x_start - scale*0.4, y_start + scale*1.2, z],
                [x_start - scale*0.3, y_start + scale*0.9, z],
                [x_start - scale*0.2, y_start + scale*0.6, z],
                [x_start - scale*0.1, y_start + scale*0.3, z],
                [x_start, y_start, z],  # Bottom point
                [x_start + scale*0.1, y_start + scale*0.3, z],
                [x_start + scale*0.2, y_start + scale*0.6, z],
                [x_start + scale*0.3, y_start + scale*0.9, z],
                [x_start + scale*0.4, y_start + scale*1.2, z]   # Top right
            ]
            ...
            points = [
                [x_start, y_start, z],
                [x_start, y_start + scale*0.3, z],
                [x_start, y_start + scale*0.6, z],
                [x_start, y_start + scale*0.9, z],
                [x_start, y_start + scale*1.2, z],  # Vertical
                [x_start + scale*0.2, y_start + scale*1.2, z],  # Top horizontal
                [x_start + scale*0.3, y_start + scale*1.1, z],
            ]
            # Add smooth curve
            import math
            for i in range(12):
                angle = math.pi/2 - i * math.pi / 11
                x = x_start + scale * 0.4 + scale * 0.35 * math.cos(angle)
                y = y_start + scale * 0.6 + scale * 0.6 * math.sin(angle)
                points.append([x, y, z])
            points.extend([
                [x_start + scale*0.3, y_start + scale*0.1, z],
                [x_start + scale*0.2, y_start, z],
                [x_start, y_start, z]  # Back to start
            ])
            ...
            points = [
                [x_start, y_start, z],
                [x_start, y_start + scale*0.4, z],
                [x_start, y_start + scale*0.8, z],
                [x_start, y_start + scale*1.2, z],  # Vertical up
                [x_start + scale*0.1, y_start + scale*1.2, z],
                [x_start + scale*0.3, y_start + scale*1.2, z],
                [x_start + scale*0.5, y_start + scale*1.2, z],  # Top horizontal
                [x_start + scale*0.3, y_start + scale*1.2, z],  # Back
                [x_start, y_start + scale*1.2, z],  # Back to vertical
                [x_start, y_start + scale*0.6, z],  # To middle
                [x_start + scale*0.1, y_start + scale*0.6, z],
            ...
            # Clear G - IMPROVED (C + horizontal bar)
            import math
            # Main C curve
            for i in range(20):
                angle = math.pi * 0.15 + i * (math.pi * 1.6) / 19
                radius = scale * 0.6
                x = x_start + radius * math.cos(angle)
                y = y_start + scale * 0.6 + radius * math.sin(angle)
                points.append([x, y, z])
            # Add horizontal line and vertical segment
            points.extend([
                [x_start + scale*0.4, y_start + scale*0.3, z],
                [x_start + scale*0.2, y_start + scale*0.3, z],
                [x_start + scale*0.2, y_start + scale*0.5, z]
            ])
            
        elif letter == 'J':
            # Clear J - IMPROVED
            points = [
                [x_start + scale*0.2, y_start + scale*1.2, z],
                [x_start + scale*0.2, y_start + scale*0.9, z],
                [x_start + scale*0.2, y_start + scale*0.6, z],
                [x_start + scale*0.2, y_start + scale*0.3, z],  # Vertical down
            ]
            # Add smooth curve at bottom
            import math
            for i in range(10):
                angle = i * math.pi / 9
                x = x_start + scale * 0.2 - scale * 0.3 * math.sin(angle)
                y = y_start + scale * 0.3 - scale * 0.3 * math.cos(angle)
                points.append([x, y, z])
            ...
            points = [
                ...
                [x_start, y_start + scale*1.2, z],  # Vertical
                [x_start, y_start + scale*0.6, z],  # Back to middle
                [x_start + scale*0.1, y_start + scale*0.7, z],  # Upper diagonal start
                [x_start + scale*0.2, y_start + scale*0.9, z],
                [x_start + scale*0.4, y_start + scale*1.2, z],  # Upper diagonal
                [x_start + scale*0.2, y_start + scale*0.9, z],  # Back
                [x_start, y_start + scale*0.6, z],  # Back to middle
                [x_start + scale*0.1, y_start + scale*0.5, z],  # Lower diagonal start
                [x_start + scale*0.2, y_start + scale*0.3, z],
                [x_start + scale*0.4, y_start, z]   # Lower diagonal
            ]
            ...
            points = [
                ...
                [x_start - scale*0.4, y_start + scale*0.9, z],
                [x_start - scale*0.4, y_start + scale*1.2, z],  # Left vertical
                [x_start - scale*0.2, y_start + scale*1.0, z],
                [x_start, y_start + scale*0.8, z],  # To center peak
                [x_start + scale*0.2, y_start + scale*1.0, z],
                [x_start + scale*0.4, y_start + scale*1.2, z],  # To right top
                [x_start + scale*0.4, y_start + scale*0.9, z],
            ...
            points = [
                [x_start - scale*0.3, y_start, z],
                [x_start - scale*0.3, y_start + scale*0.3, z],
                [x_start - scale*0.3, y_start + scale*0.6, z],
                [x_start - scale*0.3, y_start + scale*0.9, z],
                [x_start - scale*0.3, y_start + scale*1.2, z],  # Left vertical
                [x_start - scale*0.2, y_start + scale*1.0, z],  # Diagonal
                [x_start - scale*0.1, y_start + scale*0.8, z],
                [x_start, y_start + scale*0.6, z],
                [x_start + scale*0.1, y_start + scale*0.4, z],
                [x_start + scale*0.2, y_start + scale*0.2, z],
                [x_start + scale*0.3, y_start, z],
                [x_start + scale*0.3, y_start + scale*0.3, z],
                [x_start + scale*0.3, y_start + scale*0.6, z],
                [x_start + scale*0.3, y_start + scale*0.9, z],
            ...
            # Default circle for unknown letters - IMPROVED
            import math
            for i in range(20):  # More points for smoother circle
    ...
    def _generate_multilayer_movement(self, letter: str, step: int, total_steps: int, 
        ...
        """Generate NATURAL letter movements using human handwriting simulation."""
        # Use natural handwriting patterns instead of hardcoded shapes
        style = ['neat', 'casual', 'rushed'][variation % 3]
        style_params = self._get_natural_style_parameters(letter, style, variation)
        
        # Generate natural handwriting trajectory
        natural_trajectory = self._simulate_human_handwriting(letter, style_params)
        
        if not natural_trajectory or len(natural_trajectory) <= 1:
            # Fallback to simple natural movement
            return self._generate_simple_natural_movement(letter, step, total_steps, variation)
        
        # Map step to trajectory index
        progress = step / max(total_steps - 1, 1)
        trajectory_index = int(progress * (len(natural_trajectory) - 1))
        trajectory_index = min(trajectory_index, len(natural_trajectory) - 1)
        
        # Get the position from the natural trajectory (as relative movement)
        point = natural_trajectory[trajectory_index]
        base_movement = np.array([point[0] - 0.1, point[1] - 0.15])  # Make relative
        
        # Apply natural variation and style effects
        movement = self._apply_natural_writing_effects(base_movement, style_params, progress)
        
        return movement
    
    def _generate_simple_natural_movement(self, letter: str, step: int, total_steps: int, variation: int):
        """Generate simple natural movement as fallback."""
        import numpy as np
        import math
        
        progress = step / max(total_steps - 1, 1)
        
        # Basic letter characteristics without hardcoding exact shapes
        letter_val = ord(letter) - ord('A')
        base_x = 0.01 * progress + 0.002 * math.sin(progress * math.pi * 2)
        base_y = 0.005 * math.sin(progress * math.pi + letter_val * 0.3)
        
        # Add natural variation
        noise_x = np.random.normal(0, 0.001) if variation > 0 else 0
        noise_y = np.random.normal(0, 0.0005) if variation > 0 else 0
        
        return np.array([base_x + noise_x, base_y + noise_y])
    
    def _apply_natural_writing_effects(self, movement: np.ndarray, style_params: dict, progress: float):
        """Apply natural writing effects based on style parameters."""
        import numpy as np
        
        # Apply tremor (natural hand shake)
        tremor_x = np.random.normal(0, style_params['tremor'])
        tremor_y = np.random.normal(0, style_params['tremor'] * 0.5)
        
        # Apply consistency (how steady the writing is)
        consistency = style_params.get('consistency', 0.7)
        consistency_factor = 1.0 - (1.0 - consistency) * np.random.random()
        
        # Apply slant (natural writing angle)
        slant = style_params.get('slant', 0.0)
        slant_adjustment = movement[1] * slant
        
        # Combine effects
        adjusted_movement = movement.copy()
        adjusted_movement[0] += tremor_x + slant_adjustment
        adjusted_movement[1] += tremor_y
        adjusted_movement *= consistency_factor
            ...
            for letter_idx, char in enumerate(word):
                ...
                style_params = self._create_enhanced_letter_context(
                    char, current_x_offset, letter_idx, len(word), word_idx
                                        ...
                                        # Generate natural trajectory using human handwriting simulation
                trajectory = self._generate_natural_letter_for_word(char, current_x_offset)
    ...
    def _generate_natural_letter_for_word(self, char: str, x_offset: float, style: str = 'casual') -> np.ndarray:
        """Generate natural letter using human handwriting simulation."""
        # Use natural handwriting generation
        style_params = self._get_natural_style_parameters(char, style, 0)
        
        # Adjust starting position
        style_params['start_x'] = 0.1 + x_offset
        style_params['start_y'] = 0.15
        
        # Generate natural trajectory points
        natural_points = self._simulate_human_handwriting(char, style_params)
        
        if natural_points:
            # Adjust positions with offset
            adjusted_points = []
            for point in natural_points:
                adjusted_point = [
                    point[0] + x_offset,
                    point[1],
                    point[2]
            ...
            # Fallback: simple natural point
            fallback_pos = [0.1 + x_offset, 0.15, 0.02]
            return np.array([fallback_pos])
